...
void CMonsterDynamite::Explode( TraceResult *pTrace, int bitsDamageType )
{
	...
	SetThink( &CMonsterDynamite::Smoke );
...
{
	SetThink( &CMonsterDynamite::Detonate );
	SetNextThink(0);
}
...
CMonsterDynamite * CMonsterDynamite:: Shoot( entvars_t *pevOwner, Vector vecStart, Vector vecVelocity, float time )
{
	...
	pDynamite->SetTouch( &CMonsterDynamite::BounceTouch );	// Bounce if touched
	...
	pDynamite->SetThink( &CMonsterDynamite::TumbleThink );
