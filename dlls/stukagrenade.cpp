...
void CStukaGrenade::PreDetonate( void )
{
	...
	SetThink( &CStukaGrenade::Detonate );
	SetNextThink( 1 );
}
...
CStukaGrenade *CStukaGrenade::ShootContact( entvars_t *pevOwner, Vector vecStart, Vector vecVelocity )
{
	...
	pGrenade->SetThink( &CStukaGrenade::DangerSoundThink );
	...
	// Explode on contact
	pGrenade->SetTouch( &CStukaGrenade::ExplodeTouch );
