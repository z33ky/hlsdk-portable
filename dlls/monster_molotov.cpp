...
void CMonsterMolotov::Explode( TraceResult *pTrace, int bitsDamageType )
{
	...
	{
		pev->origin = pTrace->vecEndPos + (pTrace->vecPlaneNormal * Q_max(1,(pev->dmg - 24)) * 0.6);
	}
	...
	m_pFireHurt = (CBaseToggle*) CBaseToggle::Create("trigger_hurt",burn,g_vecZero,edict());
	//m_pFireHurt = (CBaseToggle*) CBaseToggle::Create("trigger_hurt",burn,pev->angles,edict());
	...
	*/
	...
	/*
	m_pBurningArea = CBaseEntity::Create("func_burning_clip",burn,pev->angles,edict());
	...
	*/
	...
	SetThink( &CMonsterMolotov::Burning );
...
{
	SetThink( &CMonsterMolotov::Detonate );
	SetNextThink(0);
}
...
CMonsterMolotov * CMonsterMolotov::ShootContact( entvars_t *pevOwner, Vector vecStart, Vector vecVelocity )
{
	...
	pMolotov->SetThink( &CMonsterMolotov::DangerSoundThink );
	...
	// Explode on contact
	pMolotov->SetTouch( &CMonsterMolotov::ExplodeTouch );
