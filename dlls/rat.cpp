...
{
	// we classify rats as insects, because everyone ignores them
...
void CRat::Touch( CBaseEntity *pOther )
{
	...
	if( m_flNextTouchTime > gpGlobals->time ) return;
	...
	EMIT_SOUND_DYN( ENT( pev ), CHAN_VOICE, "rat/rat_squeak.wav", 0.3f, ATTN_NORM, 0, 80 + RANDOM_LONG(0,39) );
...
{
	//random sound
	EMIT_SOUND_DYN( ENT( pev ), CHAN_VOICE, "rat/rat_die.wav", 0.3f, ATTN_NORM, 0, 80 + RANDOM_LONG(0,39) );
...
void CRat::MonsterThink( void )
{
	if( FNullEnt( FIND_CLIENT_IN_PVS( edict() ) ) )
		SetNextThink( RANDOM_FLOAT( 1, 1.5f ) );
	else
		SetNextThink( 0.1f );// keep monster thinking

	RunAI();

	if( !IsAlive() ) m_iMode = RAT_DEAD;
...
void CRat::PickNewDest( int iCondition )
{
	...
	} while( ( vecDest - pev->origin ).Length2D() < 128.0f );
